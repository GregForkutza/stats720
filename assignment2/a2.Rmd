...
of over dispersion is probably not warranted because of the
...
nll_fit3 <- function(params, data) {
  # Unpack params into 4 beta coefficients
...
$$ \frac{\partial log L}{\partial \beta_{r}} \equiv U(\beta_{r}) = 0$$. Firth suggested 
...
referred to as Jeffery's invariant prior. Jeffreys  prior is influential 
...
to adjust the parameters estimates in the case of our quasi-complete separation.
...
error $1.71 \times 10^{3}$ is extremely large compared to the other two methods. 
The other parameters are very similar.
